...
class CourseService {
  ...
  Future<Map<String, LessonProgress>> getUserProgress(String userId) async {
    try {
      ...
      for (var lessonId in lessonsInClass) {
        if (lessonId != 'legacy') {
          final rawModuleData = progressData[lessonId];
          final moduleData =
              rawModuleData != null
                  ? Map<String, dynamic>.from(rawModuleData as Map)
                  : <String, dynamic>{};
  ...
  Future<LessonProgress?> getSingleLessonProgress(
    ...
    try {
      ...
      final rawModuleData = progressData[lessonId];
      final moduleData = Map<String, dynamic>.from(rawModuleData as Map);

      return await _buildLessonProgress(lessonId, moduleData, classData['id']);
    } catch (e) {
  ...
  Future<LessonProgress?> _buildLessonProgress(
    ...
    try {
      ...
      if (moduleData.containsKey('preQuiz') && moduleData['preQuiz'] != null) {
        final preQuizData = Map<String, dynamic>.from(
          moduleData['preQuiz'] as Map,
        );
      ...
      print(
        'DEBUG: Built progress - Post-quiz complete: ${progress.isPostQuizComplete}',
      );
      print(
  ...
  ReadingSlide _createSingleReadingSlide(Map<String, dynamic> slideData) {
    ...
    return ReadingSlide(content: [TextContent(content)]);
  }
}
